...
| 自定义房间号 | 创建面板可填 4 位自定义号（留空则随机）；已被占用会拒绝创建。房间号不区分大小写，输入时自动转大写并过滤掉字母数字以外的字符 |
...
| `POST` | `/api/rooms` | 创建房间。`roomId` 留空 => 随机生成 4 位；填了则作为**自定义房间号**（已被占用返回 `409 ROOM_EXISTS`，格式非法返回 `400 INVALID_ROOM_ID`） |
