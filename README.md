...
| 回收倒计时横幅 | **A 端离线超过 30 分钟**后，页面顶部显示横幅，同时告知离线时长与距房间回收还剩多少时间（逐秒递减）。一旦有设备进入「查看」模式，心跳刷新，横幅自动消失 |
...
| `roomRecycleMs` | 24 小时（设 `0` / `null` 即**关闭回收**） | 4.1 |
...
11. **房间回收回 24 小时**：PRD 4.1 即为 24 小时（中途曾缩短到 6 小时，现已改回）。
    为避免「设备离线很久却毫无感知」，前端在 **A 端离线超过 30 分钟**后会显示
    回收倒计时横幅，详见 5 节。要调整阈值只需改 `src/config.js` 的 `roomRecycleMs`
    与 `public/js/app.js` 的 `IDLE_WARN_MS`。
12. **允许关闭房间回收**：把 `src/config.js` 的 `roomRecycleMs` 设为 `0` 或 `null`，
    房间就不会再因 A 端离线而被删除（启动日志会明确打印「已关闭」）。
    此时横幅仍会提示离线时长，但不显示倒计时。

    > ⚠️ 关闭后房间会一直留在数据目录里，而且**短房间号的号段不会被释放**。
    > 家庭自用无影响，但长期不清理需要留意磁盘占用。
